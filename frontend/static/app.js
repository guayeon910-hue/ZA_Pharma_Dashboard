/**
 * UPharma Export AI — 남아공(ZA) 대시보드 스크립트
 * ═══════════════════════════════════════════════════════════════
 *
 * 기능 목록:
 *   §1  상수 & 전역 상태
...
const INN_MAP = {
  ...
  gadvoa_inj:        'Gadobutrol PFS',
  sereterol_activair:'Fluticasone / Salmeterol',
  omethyl_cutielet:  'Omega-3 EE 2g',
  rosumeg_combigel:  'Rosuvastatin + Omega-3',
  atmeg_combigel:    'Atorvastatin + Omega-3',
  ciloduo:           'Cilostazol + Rosuvastatin',
  gastiin_cr:        'Mosapride CR',
};
...
// P2 3열 시나리오용 원본 데이터
let _p2ScenarioRaw = { agg: 0, avg: 0, cons: 0, zar_usd: 0, zar_krw: 0 };
...
async function loadExchange() {
  ...
  try {
    ...
    if (rateEl) {
      const fmt = Number(data.zar_krw).toLocaleString('ko-KR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      rateEl.innerHTML = `${fmt}<span style="font-size:14px;margin-left:4px;color:var(--muted);font-weight:700;">원</span>`;
    }

    // 서브 그리드 (USD/KRW + ZAR 연관 환율)
    const subEl = document.getElementById('exchange-sub');
    if (subEl) {
      const fmtUsd = Number(data.usd_krw).toLocaleString('ko-KR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const fmtZarUsd = Number(data.zar_usd).toFixed(4);
      subEl.innerHTML = `
        ...
        <div class="irow" style="margin:0">
          <div style="font-size:10.5px;color:var(--muted);margin-bottom:3px;">ZAR / USD</div>
          <div style="font-size:15px;font-weight:900;color:var(--navy);">${fmtZarUsd}</div>
        </div>
      `;
    }
...
const TODO_LS_KEY    = 'za_upharma_todos_v1';
...
function _makeP2Defaults() {
  return {
    public: [
      { key: 'base_price', label: '기준 입찰가 (SEP/MHPL)', value: 0, type: 'abs_input', unit: 'ZAR', step: 1, min: 0, max: 99999, enabled: true, fixed: false, expanded: false, hint: 'SEP 또는 MHPL 입찰 기준가 (R)', rationale: '남아공 공공 시장은 MHPL 입찰 경쟁이 핵심입니다.' },
      { key: 'exchange', label: '환율 (USD→ZAR)', value: 18.0, type: 'abs_input', unit: 'rate', step: 0.1, min: 0.0001, max: 999, enabled: true, fixed: false, expanded: false, hint: 'USD 입력 시 ZAR 전환 비율', rationale: '실시간 환율을 반영해 환차 리스크를 줄입니다.' },
    ...
    private: [
      { key: 'base_het', label: '민간 기준가 (Clicks/Dis-Chem)', value: 0, type: 'abs_input', unit: 'ZAR', step: 1, min: 0, max: 99999, enabled: true, fixed: false, expanded: false, hint: 'Clicks/Dis-Chem 소매 기준가 (R)', rationale: '남아공 민간 시장은 소매 가격 역산이 중요합니다.' },
      { key: 'exchange', label: '환율 (USD→ZAR)', value: 18.0, type: 'abs_input', unit: 'rate', step: 0.1, min: 0.0001, max: 999, enabled: true, fixed: false, expanded: false, hint: 'USD 입력 시 ZAR 전환 비율', rationale: '실시간 환율 반영으로 가격 정합성을 유지합니다.' },
      { key: 'vat', label: 'VAT 공제 (÷1.15)', value: 15, type: 'gst_fixed', unit: '%', step: 0, min: 15, max: 15, enabled: true, fixed: true, expanded: false, hint: '남아공 VAT 15% 고정', rationale: '민간 소비자 가격에서 세금을 분리합니다.' },
      { key: 'retail', label: '소매 마진율', value: 40, type: 'pct_deduct', unit: '%', step: 1, min: 0, max: 99999, enabled: true, fixed: false, expanded: false, hint: 'Clicks/Dis-Chem 체인 마진 차감', rationale: '채널별 마진 차이를 반영합니다.' },
...
function recalcP2Col(col) {
  ...
  const usd = _p2ScenarioRaw.zar_usd > 0 ? (price * _p2ScenarioRaw.zar_usd).toFixed(2) : '—';
  const krw = _p2ScenarioRaw.zar_krw > 0 ? Math.round(price * _p2ScenarioRaw.zar_krw).toLocaleString('ko-KR') : '—';
...
function renderP2ColOptions(col, showAddForm) {
  ...
  const typeLabel = { pct_add: '% 가산', pct_deduct: '% 차감', abs_add: 'ZAR 가산' };
    ...
    html += `
      <div class="p2c-opt-row p2c-add-row">
        ...
        <select class="p2c-opt-type-select" id="p2c-newtype-${col}">
          ...
          <option value="abs_add">ZAR 가산</option>
        </select>
...
function _renderP2AiResult(data) {
  ...
  _setText('p2r-ref-price-text',
    extracted.ref_price_text || (extracted.ref_price_zar != null ? `R ${Number(extracted.ref_price_zar).toFixed(2)}` : '추출값 없음'));
  const krwRate = rates.zar_krw;
  const usdRate = rates.zar_usd;
  let rateText = '환율 정보 없음';
  if (krwRate) {
    rateText = `1 ZAR = ${Number(krwRate).toFixed(4)} KRW`;
    if (usdRate) rateText += ` / ${Number(usdRate).toFixed(4)} USD`;
  }
  _setText('p2r-exchange', rateText);

  // 최종 권고가
  _setText('p2r-final-price', `R ${Number(analysis.final_price_zar || 0).toFixed(2)}`);
        ...
        return `
          <div class="p2-scenario p2-scenario--${cls}">
            <div class="p2-scenario-top">
              ...
              <span class="p2-scenario-price">R ${Number(s.price_zar || 0).toFixed(2)}</span>
            </div>
          </div>`;
      }).join('');
    } else {
  ...
  // ── 3열 시나리오 UI 채우기 ──────────────────────────────
  const zarUsd = rates.zar_usd ? Number(rates.zar_usd) : 0;
  const zarKrw = rates.zar_krw ? Number(rates.zar_krw) : 0;

  const cols = ['agg', 'avg', 'cons'];
  scenarios.forEach((s, i) => {
    const col     = cols[i];
    if (!col) return;
    const priceZar = Number(s.price_zar || 0);
    _p2ScenarioRaw[col]     = priceZar;
    _p2ScenarioRaw.zar_usd  = zarUsd;
    _p2ScenarioRaw.zar_krw  = zarKrw;

    const refBase = extracted.ref_price_zar != null ? Number(extracted.ref_price_zar) : 0;
    const refLabel = refBase > 0
      ? `Retail base: R ${(refBase * (i === 0 ? 1.3 : i === 1 ? 1.0 : 0.7)).toFixed(2)}`
      : `Retail base: — ZAR`;
    ...
    if (priceEl)   priceEl.textContent = priceZar.toFixed(2);
    if (baseInput) baseInput.value     = priceZar.toFixed(2);
    if (subEl) {
      const usd = zarUsd > 0 ? (priceZar * zarUsd).toFixed(2) : '—';
      const krw = zarKrw > 0 ? Math.round(priceZar * zarKrw).toLocaleString('ko-KR') : '—';
  ...
  if (scenarios.length >= 3) {
    const prices = scenarios.map(s => Number(s.price_zar || 0)).sort((a, b) => a - b);
    _setText('p2-dist-p25', `R ${prices[0].toFixed(2)}`);
    _setText('p2-dist-med', `R ${prices[1].toFixed(2)}`);
    _setText('p2-dist-p75', `R ${prices[2].toFixed(2)}`);
  }
...
function _p2FillExchangeRate() {
  ...
  if (!rates) return;
  const zarUsd = Number(rates.zar_usd);
  if (!zarUsd || zarUsd <= 0) return;
  // ZAR 기준: 1 USD = (1/zarUsd) ZAR
  const usdToZar = Number((1 / zarUsd).toFixed(2));
  ['public', 'private'].forEach((seg) => {
    const opt = _p2Manual[seg].find((x) => x.key === 'exchange');
    if (opt) opt.value = usdToZar;
  });
}
...
function _p2FillBaseFromReport() {
  ...
  if (!report) return;
  // 1순위: 저장된 숫자형 ZAR 값
  const numHint = report.zar_hint || report.sep_zar;
  const hint = (numHint != null && !Number.isNaN(Number(numHint)) && Number(numHint) > 0)
    ? Number(numHint)
    : _extractZarHint(report.price_hint || '');
...
function _extractZarHint(text) {
  const src = String(text || '');
  // R 1,234.56 또는 ZAR 1234 또는 R1234
  const mRange = src.match(/(?:R|ZAR)\s*([0-9,]+(?:\.[0-9]+)?)\s*[~\-–]\s*([0-9,]+(?:\.[0-9]+)?)/i);
  if (mRange) return (parseFloat(mRange[1].replace(/,/g,'')) + parseFloat(mRange[2].replace(/,/g,''))) / 2;
  const mSingle = src.match(/(?:R|ZAR)\s*([0-9,]+(?:\.[0-9]+)?)/i);
  if (mSingle) return parseFloat(mSingle[1].replace(/,/g,''));
  // USD 폴백: 환율 18 적용
  const mUsd = src.match(/(?:\$|USD\s+)([0-9]+(?:\.[0-9]+)?)/i);
  if (mUsd) return Number(mUsd[1]) * 18;
  return NaN;
}
...
function _calcP2Manual() {
  ...
  if (seg === 'public') {
    ...
    const parts = [`R ${base.toFixed(2)}`, `× ${ex.toFixed(2)}`, `× ${ratio}%`];
    options.forEach((opt) => {
      if (opt.type === 'pct_add_custom') {
        ...
        parts.push(`+ R ${Number(opt.value).toFixed(2)}`);
      }
    });
    return { kup: Math.max(price, 0), formulaStr: `${parts.join('  ')}  =  KUP  R ${Math.max(price, 0).toFixed(2)}` };
  }
  ...
  return { kup: Math.max(price, 0), formulaStr: `${(parts.join('  ') || 'R 0.00')}  =  KUP  R ${Math.max(price, 0).toFixed(2)}` };
}
...
function _renderP2Manual() {
  ...
  const aggFormula  = `KUP R ${calc.kup.toFixed(2)} × 0.90 = R ${agg.toFixed(2)}`;
  const avgFormula  = `KUP R ${avg.toFixed(2)} (기준가 그대로)`;
  const consFormula = `KUP R ${calc.kup.toFixed(2)} × 1.10 = R ${cons.toFixed(2)}`;
...
function _p2OptionCardHtml(opt) {
  ...
  // 단위 표시
  const unitLabel = opt.unit === '%' ? '%' : opt.unit === 'rate' ? '' : 'ZAR';
  ...
  section.innerHTML = `
    <div class="p2-custom-add-row">
      ...
      <select class="p2-custom-type-select" id="p2c-type">
        ...
        <option value="abs_add_custom">ZAR 가산</option>
      </select>
...
function _zarPriceLineFromApi(result) {
  const sep = result.sep_zar;
  if (sep != null && sep !== '') {
    const num = Number(sep);
    if (!Number.isNaN(num)) return `SEP R ${num.toFixed(2)}`;
  }
  const hint = String(result.price_hint || '').trim();
  if (hint) return hint;
  return '참고 가격 정보 없음';
}
...
async function loadNews() {
  ...
  try {
    const res  = await fetch('/api/za/news');
    const data = await res.json();
...
const P3_PRODUCT_LABELS = {
  ...
  gastiin_cr:        'Gastiin CR · Mosapride Citrate 15mg',
  omethyl_cutielet:  'Omethyl Cutielet · Omega-3 EE 2g',
...
async function runP3Pipeline() {
  ...
  const product = document.getElementById('product-select')?.value || 'sereterol_activair';
...
function _renderP3Cards(buyers) {
  ...
  wrap.innerHTML = buyers.map((b, i) => {
    ...
    const priLabel  = pri === 1 ? '성분 일치' : 'South Africa';
...
function showBuyerDetail(idx) {
  ...
  const e = b.enriched || {};
  const rankEmoji = ['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣','7️⃣','8️⃣','9️⃣','🔟'];
  const priLabel = b.priority === 1 ? '성분 일치' : 'South Africa';
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   §13. Leaflet 지도 초기화 (남아프리카공화국)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */

(function initZaMap() {
  const el = document.getElementById('za-map');
  if (!el || typeof L === 'undefined') return;
  const map = L.map('za-map', { zoomControl: true, scrollWheelZoom: false });
  ...
  const cities = [
    [-26.2041, 28.0473, '<b>요하네스버그</b><br>최대 상업도시 · Clicks/Dis-Chem 본사'],
    [-25.7461, 28.1881, '<b>프리토리아</b><br>행정수도 · SAHPRA 규제청 소재'],
    [-33.9249, 18.4241, '<b>케이프타운</b><br>입법수도 · 주요 항구'],
    [-29.8587, 31.0218, '<b>더반</b><br>쾌즐루나탈 주도 · 수출 항구'],
  ];
  cities.forEach(([lat, lng, popup]) => {
